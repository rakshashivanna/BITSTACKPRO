...
module nbit_stack #(
    ...
    parameter DEPTH = 850  // Depth of stack 
)(
    input wire clk,
    input wire reset,
    input wire push,
    ...
    input wire [WIDTH-1:0] scan_in,  // Input for scan chain
    output reg [WIDTH-1:0] scan_out        // Output for scan chain
);
    ...
    // Logic to handle data_out in both normal and test mode
    ...
    always @(posedge clk or posedge reset) begin
        if (reset) begin
            scan_out <= 0;  // Reset scan_out during reset
        end else if (test_mode) begin
            scan_out <= scan_reg_data_out;  // Output the entire shifted data during test mode
        end else begin
            scan_out <= 1'b1;  // Default behavior for normal mode
        end
    end
